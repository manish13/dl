...
def make_universe():
    ...
    u.to_parquet(f'{ROOT}universe.parquet')
...
def make_price():
    ...
    u = CharacterisitcMaker().universe()
    p.reindex(columns=u.columns).to_parquet(f'{ROOT}price.parquet')

def make_volume():
    a = techfac.get_price_data()
    p = techfac.get_data(a.volume).fillna(0)
    u = CharacterisitcMaker().universe()
    p.reindex(columns=u.columns).to_parquet(f'{ROOT}volume.parquet')

def make_mktcap():
    a = techfac.get_price_data()
    p = techfac.get_data(a.marketCap).ffill(limit=1)
    u = CharacterisitcMaker().universe()
    p.reindex(columns=u.columns).to_parquet(f'{ROOT}mktcap.parquet')

def make_ff3():
    prc = pd.read_parquet(f'{ROOT}price.parquet')
    a = pd.read_csv(f'{ROOT}FF3.csv', index_col=0, parse_dates=True)
    a.reindex(index=prc.index).to_parquet(f'{ROOT}ff3.parquet')
...
class CharacterisitcMaker:
    ...
    def universe(self):
        return pd.read_parquet(f'{ROOT}universe.parquet')

    def returns(self):
        return pd.read_parquet(f'{ROOT}returns.parquet')
    
    def price(self):
        return pd.read_parquet(f'{ROOT}price.parquet')
    
    def volume(self):
        return pd.read_parquet(f'{ROOT}volume.parquet')
    
    def mktcap(self):
        return pd.read_parquet(f'{ROOT}mktcap.parquet')
    
    def ff3(self):
        return pd.read_parquet(f'{ROOT}ff3.parquet')[['Mkt-RF','SMB','HML']]
    
    def market(self):
        ff3 = pd.read_parquet(f'{ROOT}ff3.parquet')
    ...
    def __call__(self, save=False):
        if self.name in Technical:
            feature = techfac.shapify(getattr(techfac, self.name)(*self.get_params()).ffill(), self.universe())
            if save:
                feature.resample('M').last().to_parquet(f'{ROOT}{self.name}.parquet')
            return feature
        else:
