...
def dl_alpha(data, layer_size, para):
    ...
    z = tf.keras.Input(name='z', shape=(n, p), dtype=tf.dtypes.float32)
    ...
    target = tf.keras.Input(name='target', shape=(port_n,), dtype=tf.dtypes.float32)
    # m = tf.placeholder(tf.float32, [None, ff_n])
    m = tf.keras.Input(name='m', shape=(ff_n,), dtype=tf.dtypes.float32)
    ...
    with tf.compat.v1.name_scope('sorting_network'):
        ...
        layer_number_tanh = len(layer_size)
