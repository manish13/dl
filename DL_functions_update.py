    ...
    with tf.compat.v1.name_scope('sorting_network'):
        ...
        for i in range(layer_number_tanh):
            ...
            if i < layer_number_tanh -1:
                weights_l1 += (tf.reduce_sum(input_tensor=tf.abs(weights)) - tf.reduce_sum(input_tensor=tf.abs(tf.linalg.diag_part(weights))))

        # softmax for factorweight
        mean, var = tf.nn.moments(x=layers_1[-1],axes=1,keepdims=True)
